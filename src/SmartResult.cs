...
/// </summary>
public readonly record struct SmartResult<TValue> : ISmartResult<TValue>
{
    private readonly TValue? _value = default;
    private readonly List<Error>? _errors = null;

    private static readonly Error NoFirstError = Error.Unexpected(
        code: "SmartResult.NoFirstError",
        description: "First error cannot be retrieved from a successful SmartResult.");

    private static readonly Error NoErrors = Error.Unexpected(
        code: "SmartResult.NoErrors",
        description: "Error list cannot be retrieved from a successful SmartResult.");
    ...
    /// <summary>
    /// Creates an <see cref="SmartResult{TValue}"/> from a list of errors.
    /// </summary>
    public static SmartResult<TValue> From(List<Error> errors)
    {
        return errors;
    }
    ...
    private SmartResult(Error error)
    {
        _errors = new List<Error> { error };
        IsError = true;
    }

    private SmartResult(List<Error> errors)
    {
        _errors = errors;
        IsError = true;
    }

    private SmartResult(TValue value)
    {
        _value = value;
        IsError = false;
    }
    ...
    /// <summary>
    /// Creates an <see cref="SmartResult{TValue}"/> from a value.
    /// </summary>
    public static implicit operator SmartResult<TValue>(TValue value)
    {
        return new SmartResult<TValue>(value);
    }

    /// <summary>
    /// Creates an <see cref="SmartResult{TValue}"/> from an error.
    /// </summary>
    public static implicit operator SmartResult<TValue>(Error error)
    {
        return new SmartResult<TValue>(error);
    }

    /// <summary>
    /// Creates an <see cref="SmartResult{TValue}"/> from a list of errors.
    /// </summary>
    public static implicit operator SmartResult<TValue>(List<Error> errors)
    {
        return new SmartResult<TValue>(errors);
    }

    /// <summary>
    /// Creates an <see cref="SmartResult{TValue}"/> from a list of errors.
    /// </summary>
    public static implicit operator SmartResult<TValue>(Error[] errors)
    {
        return new SmartResult<TValue>(errors.ToList());
    }

    /// <summary>
    /// Executes the appropriate action based on the state of the <see cref="SmartResult{TValue}"/>.
    ...
    /// <summary>
    /// Asynchronously executes the appropriate action based on the state of the <see cref="SmartResult{TValue}"/>.
    ...
    /// <summary>
    /// Executes the appropriate function based on the state of the <see cref="SmartResult{TValue}"/>.
    ...
    /// <summary>
    /// Asynchronously executes the appropriate function based on the state of the <see cref="SmartResult{TValue}"/>.
...
/// <summary>
/// Provides utility methods for creating instances of <see ref="SmartResult{T}"/>.
/// </summary>
public static class SmartResult
{
    /// <summary>
    /// Creates an <see ref="SmartResult{TValue}"/> instance from a value.
    /// </summary>
    /// <typeparam name="TValue">The type of the value.</typeparam>
    /// <param name="value">The value from which to create an SmartResult instance.</param>
    /// <returns>An <see ref="SmartResult{TValue}"/> instance containing the specified value.</returns>
